...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    for event_data in log['events']:
        ...
        if event_data["type"] == "call":
            ...
            our_call = Call(src_num, dst_num, new_date, durtn, src_loc, dst_loc)
            caller = find_customer_by_number(our_call.src_number, customer_list)
            callee = find_customer_by_number(our_call.dst_number, customer_list)
