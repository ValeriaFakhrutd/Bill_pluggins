        ...
        try:
            ...
            for customer in customers:
                if customer.get_id() == int(filter_string):
                    id_list.append(customer)
            if len(id_list) == 0:
                return data
            numbers = id_list[0].get_phone_numbers()
            for call in data:
                if call.dst_number in numbers or call.src_number in numbers:
                    answer.append(call)
            return answer
        except Exception:
            return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if not filter_string[1:].isdigit():
            return data
        answer = []
        number = int(filter_string[1:])
        if filter_string[0] == "L":
            for call in data:
                if call.duration < number:
                    answer.append(call)
    ...
    def apply(self, customers: List[Customer],
        ...
        for el in loc:
            try:
                ...
                return data
        if float(loc[0]) > float(loc[2]) or float(loc[1]) > float(loc[3]):
            return data
        if float(loc[0]) < -79.697878 or float(loc[2]) > -79.196382:
            return data
        if float(loc[1]) < 43.576959 or float(loc[3]) > 43.799568:
            return data
        answer = []
        for call in data:
            if (float(loc[0]) <= call.src_loc[0] <= float(loc[2]))\
                    and (float(loc[1]) <= call.src_loc[1] <= float(loc[3])):
                answer.append(call)
            elif (float(loc[0]) <= call.dst_loc[0] <= float(loc[2]))\
                    and (float(loc[1]) <= call.dst_loc[1] <= float(loc[3])):
                answer.append(call)
        return answer
